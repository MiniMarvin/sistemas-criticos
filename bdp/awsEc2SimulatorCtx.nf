...
THEORY ListConstantsX IS
  List_Valuable_Constants(Machine(awsEc2SimulatorCtx))==(allocated,spot,vmLimit);
  Inherited_List_Constants(Machine(awsEc2SimulatorCtx))==(?);
  List_Constants(Machine(awsEc2SimulatorCtx))==(allocated,spot,vmLimit)
END
&
THEORY ListSetsX IS
...
THEORY ListPropertiesX IS
  ...
  List_Properties(Machine(awsEc2SimulatorCtx))==(allocated: VM_CATEGORIES & spot: VM_CATEGORIES & vmLimit: NAT1 & USERS: FIN(INTEGER) & not(USERS = {}) & MACHINE_RESOURCES: FIN(INTEGER) & not(MACHINE_RESOURCES = {}) & VIRTUAL_MACHINES: FIN(INTEGER) & not(VIRTUAL_MACHINES = {}) & VM_CATEGORIES: FIN(INTEGER) & not(VM_CATEGORIES = {}))
END
&
...
THEORY ConstantsEnvX IS
  Constants(Machine(awsEc2SimulatorCtx)) == (Type(allocated) == Cst(atype(VM_CATEGORIES,?,?));Type(spot) == Cst(atype(VM_CATEGORIES,?,?));Type(vmLimit) == Cst(btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
