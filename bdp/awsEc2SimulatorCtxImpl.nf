...
THEORY ListInvariantX IS
  ...
  Abstract_List_Invariant(Implementation(awsEc2SimulatorCtxImpl))==(USERS: FIN(USERS) & VM_CATEGORIES: FIN(VM_CATEGORIES) & VIRTUAL_MACHINES: FIN(VIRTUAL_MACHINES));
...
THEORY ListConstantsX IS
  List_Valuable_Constants(Implementation(awsEc2SimulatorCtxImpl))==(allocated,spot,vmLimit);
  Inherited_List_Constants(Implementation(awsEc2SimulatorCtxImpl))==(allocated,spot,vmLimit);
...
THEORY ListPropertiesX IS
  Abstract_List_Properties(Implementation(awsEc2SimulatorCtxImpl))==(allocated: VM_CATEGORIES & spot: VM_CATEGORIES & vmLimit: NAT1 & USERS: FIN(INTEGER) & not(USERS = {}) & MACHINE_RESOURCES: FIN(INTEGER) & not(MACHINE_RESOURCES = {}) & VIRTUAL_MACHINES: FIN(INTEGER) & not(VIRTUAL_MACHINES = {}) & VM_CATEGORIES: FIN(INTEGER) & not(VM_CATEGORIES = {}));
...
THEORY ListValuesX IS
  ...
  Values_Subs(Implementation(awsEc2SimulatorCtxImpl))==(MACHINE_RESOURCES,VIRTUAL_MACHINES,vmLimit,VM_CATEGORIES,allocated,spot,USERS: 0..100,1..1000,10000,{0,1},0,1,1..100);
  List_Values(Implementation(awsEc2SimulatorCtxImpl))==(MACHINE_RESOURCES = 0..100;VIRTUAL_MACHINES = 1..1000;vmLimit = 10000;VM_CATEGORIES = {0,1};allocated = 0;spot = 1;USERS = 1..100)
END
&
...
THEORY InheritedEnvX IS
  Constants(Implementation(awsEc2SimulatorCtxImpl))==(Type(allocated) == Cst(atype(VM_CATEGORIES,?,?));Type(spot) == Cst(atype(VM_CATEGORIES,?,?));Type(vmLimit) == Cst(btype(INTEGER,?,?)))
END
&
...
THEORY ConstantsEnvX IS
  Constants(Implementation(awsEc2SimulatorCtxImpl)) == (Type(vmLimit) == Cst(btype(INTEGER,?,?));Type(spot) == Cst(btype(INTEGER,?,?));Type(allocated) == Cst(btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
