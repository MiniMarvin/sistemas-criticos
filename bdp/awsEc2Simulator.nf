...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Machine(awsEc2Simulator),removeVirtualMachine)==(vm: virtualMachines & vm: dom(virtualMachineProperties) & caller: admins\/{virtualMachineProperties(vm)'owner});
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(awsEc2Simulator),removeVirtualMachine)==(vm: virtualMachines & vm: dom(virtualMachineProperties) & caller: admins\/{virtualMachineProperties(vm)'owner} | virtualMachines,allocatedVirtualMachines,spotVirtualMachines,finishedInstanceCost:=virtualMachines-{vm},allocatedVirtualMachines-{vm},spotVirtualMachines-{vm},finishedInstanceCost<+{virtualMachineProperties(vm)'owner|->finishedInstanceCost(virtualMachineProperties(vm)'owner)+instanceRating(virtualMachineProperties(vm)'category)'cpu*virtualMachineProperties(vm)'cpu*(currentTime-virtualMachineProperties(vm)'startTime)+instanceRating(virtualMachineProperties(vm)'category)'hdd*virtualMachineProperties(vm)'hdd*(currentTime-virtualMachineProperties(vm)'startTime)+instanceRating(virtualMachineProperties(vm)'category)'mem*virtualMachineProperties(vm)'mem*(currentTime-virtualMachineProperties(vm)'startTime)});
  Expanded_List_Substitution(Machine(awsEc2Simulator),addSpotVirtualMachine)==(cpu: NAT1 & hdd: NAT1 & mem: NAT1 & client: clients | @(machine,vm).(vm: VIRTUAL_MACHINES & vm/:virtualMachines & vm/:spotVirtualMachines & vm/:allocatedVirtualMachines & machine: machineResources & machine: dom(machineResourceProperties) & cpu<=machineResourceProperties(machine)'cpu-SIGMA(virtualMachine).(virtualMachine: virtualMachines & virtualMachineProperties(virtualMachine)'residentMachine = machine | virtualMachineProperties(virtualMachine)'cpu) & hdd<=machineResourceProperties(machine)'hdd-SIGMA(virtualMachine).(virtualMachine: virtualMachines & virtualMachineProperties(virtualMachine)'residentMachine = machine | virtualMachineProperties(virtualMachine)'hdd) & mem<=machineResourceProperties(machine)'mem-SIGMA(virtualMachine).(virtualMachine: virtualMachines & virtualMachineProperties(virtualMachine)'residentMachine = machine | virtualMachineProperties(virtualMachine)'mem) ==> virtualMachines,spotVirtualMachines,virtualMachineProperties:=virtualMachines\/{vm},spotVirtualMachines\/{vm},virtualMachineProperties<+{vm|->rec(residentMachine>>machine,owner>>client,category>>allocated,startTime>>currentTime,cpu>>cpu,hdd>>hdd,mem>>mem)}));
  ...
  Expanded_List_Substitution(Machine(awsEc2Simulator),removeClient)==(user: USERS & user: clients & caller: admins | clients,finishedInstanceCost:=clients-{user},{user}<<|finishedInstanceCost);
  ...
  List_Substitution(Machine(awsEc2Simulator),addSpotVirtualMachine)==(ANY machine,vm WHERE vm: VIRTUAL_MACHINES & vm/:virtualMachines & vm/:spotVirtualMachines & vm/:allocatedVirtualMachines & machine: machineResources & machine: dom(machineResourceProperties) & cpu<=machineResourceProperties(machine)'cpu-SIGMA(virtualMachine).(virtualMachine: virtualMachines & virtualMachineProperties(virtualMachine)'residentMachine = machine | virtualMachineProperties(virtualMachine)'cpu) & hdd<=machineResourceProperties(machine)'hdd-SIGMA(virtualMachine).(virtualMachine: virtualMachines & virtualMachineProperties(virtualMachine)'residentMachine = machine | virtualMachineProperties(virtualMachine)'hdd) & mem<=machineResourceProperties(machine)'mem-SIGMA(virtualMachine).(virtualMachine: virtualMachines & virtualMachineProperties(virtualMachine)'residentMachine = machine | virtualMachineProperties(virtualMachine)'mem) THEN virtualMachines:=virtualMachines\/{vm} || spotVirtualMachines:=spotVirtualMachines\/{vm} || virtualMachineProperties(vm):=rec(residentMachine>>machine,owner>>client,category>>allocated,startTime>>currentTime,cpu>>cpu,hdd>>hdd,mem>>mem) END);
  List_Substitution(Machine(awsEc2Simulator),removeVirtualMachine)==(virtualMachines:=virtualMachines-{vm} || allocatedVirtualMachines:=allocatedVirtualMachines-{vm} || spotVirtualMachines:=spotVirtualMachines-{vm} || finishedInstanceCost(virtualMachineProperties(vm)'owner):=finishedInstanceCost(virtualMachineProperties(vm)'owner)+instanceRating(virtualMachineProperties(vm)'category)'cpu*virtualMachineProperties(vm)'cpu*(currentTime-virtualMachineProperties(vm)'startTime)+instanceRating(virtualMachineProperties(vm)'category)'hdd*virtualMachineProperties(vm)'hdd*(currentTime-virtualMachineProperties(vm)'startTime)+instanceRating(virtualMachineProperties(vm)'category)'mem*virtualMachineProperties(vm)'mem*(currentTime-virtualMachineProperties(vm)'startTime));
