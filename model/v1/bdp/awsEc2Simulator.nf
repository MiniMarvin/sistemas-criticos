...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(awsEc2Simulator),tickTimer)==(btrue | currentTime:=currentTime+1);
  ...
  List_Substitution(Machine(awsEc2Simulator),tickTimer)==(currentTime:=currentTime+1);
  ...
  List_Substitution(Machine(awsEc2Simulator),addAllocatedVirtualMachine)==(ANY machine,vm WHERE vm: VIRTUAL_MACHINES & vm/:virtualMachines & vm/:spotVirtualMachines & vm/:allocatedVirtualMachines & machine: machineResources & machine: dom(machineResourceProperties) & cpu<=machineResourceProperties(machine)'cpu-SIGMA(virtualMachine).(virtualMachine: virtualMachines & virtualMachineProperties(virtualMachine)'residentMachine = machine & virtualMachineProperties(virtualMachine)'category = allocated | virtualMachineProperties(virtualMachine)'cpu) & hdd<=machineResourceProperties(machine)'hdd-SIGMA(virtualMachine).(virtualMachine: virtualMachines & virtualMachineProperties(virtualMachine)'residentMachine = machine & virtualMachineProperties(virtualMachine)'category = allocated | virtualMachineProperties(virtualMachine)'hdd) & mem<=machineResourceProperties(machine)'mem-SIGMA(virtualMachine).(virtualMachine: virtualMachines & virtualMachineProperties(virtualMachine)'residentMachine = machine & virtualMachineProperties(virtualMachine)'category = allocated | virtualMachineProperties(virtualMachine)'mem) THEN virtualMachines:=(virtualMachines\/{vm})-{vm | vm: dom(virtualMachineProperties) & virtualMachineProperties(vm)'category = spot & virtualMachineProperties(vm)'residentMachine = machine} || allocatedVirtualMachines:=allocatedVirtualMachines\/{vm} || virtualMachineProperties:={vm | vm: dom(virtualMachineProperties) & virtualMachineProperties(vm)'category = spot & virtualMachineProperties(vm)'residentMachine = machine}<<|(virtualMachineProperties\/{vm|->rec(residentMachine>>machine,owner>>client,category>>allocated,startTime>>currentTime,cpu>>cpu,hdd>>hdd,mem>>mem)}) || spotVirtualMachines:=spotVirtualMachines-{vm | vm: dom(virtualMachineProperties) & virtualMachineProperties(vm)'category = spot & virtualMachineProperties(vm)'residentMachine = machine} END);
...
THEORY OperationsEnvX IS
  ...
  Observers(Machine(awsEc2Simulator)) == (Type(getBillingForUser) == Cst(btype(INTEGER,?,?),atype(USERS,?,?));Type(getBillingForRunningVM) == Cst(btype(INTEGER,?,?),atype(VIRTUAL_MACHINES,?,?));Type(listVmsForUser) == Cst(SetOf(atype(VIRTUAL_MACHINES,?,?)),SetOf(atype(VM_CATEGORIES,?,?))*atype(USERS,?,?));Type(getAllocatedCpuOnMachine) == Cst(btype(INTEGER,?,?),atype(MACHINE_RESOURCES,?,?));Type(getAllocatedVmsOnMachine) == Cst(SetOf(atype(VIRTUAL_MACHINES,?,?)),atype(MACHINE_RESOURCES,?,?));Type(listResources) == Cst(SetOf(atype(MACHINE_RESOURCES,?,?)*rtype(((cpu: btype(INTEGER,1,MAXINT)),hdd: btype(INTEGER,1,MAXINT)),mem: btype(INTEGER,1,MAXINT))),No_type);Type(listClients) == Cst(SetOf(atype(USERS,?,?)),atype(USERS,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
